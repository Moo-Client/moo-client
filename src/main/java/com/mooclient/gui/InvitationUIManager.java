...
public class InvitationUIManager {
    ...
    private void renderFloatingCenter(DrawContext context, Interaction inv, int sw, int sh) {
        ...
        String desc = "§b" + inviter + " §fzaprasza do §e" + emoteName;
        ...
        // Przyciski akcji
        String acceptText = "§a[" + getAcceptKeyName() + "] " + MooLanguage.get("invitation_accept");
        String declineText = "§c[" + getDeclineKeyName() + "] " + MooLanguage.get("invitation_decline");
        context.drawTextWithShadow(client.textRenderer, acceptText, iconX + 40, y + 44, 0xFF55FF55);
        context.drawTextWithShadow(client.textRenderer, declineText, iconX + 130, y + 44, 0xFFFF5555);
    }
    ...
    private void renderCompactToast(DrawContext context, Interaction inv, int sw, int sh) {
        ...
        String line2 = "§a[" + getAcceptKeyName() + "] Tak  §7|  §c[" + getDeclineKeyName() + "] Nie";
    ...
    private void renderMooSleekCard(DrawContext context, Interaction inv, int sw, int sh) {
        ...
        // Header
        context.drawTextWithShadow(client.textRenderer, "§6★ §fINTERAKCJA MOO CLIENT", x + 10, y + 6, 0xFFFFFFFF);

        // Treść
        String msg = "§fGracz §b" + inv.getInitiatorName() + " §foferuje §e" + emoteName;
        ...
        // Przycisk Akceptuj
        context.fill(x + 10, btnY, x + 10 + btnW, btnY + btnH, 0xCC1A3824);
        drawBorder(context, x + 10, btnY, btnW, btnH, 0xFF55FF55);
        context.drawCenteredTextWithShadow(client.textRenderer, "§a[" + getAcceptKeyName() + "] AKCEPTUJ", x + 10 + btnW / 2, btnY + 6, 0xFFFFFFFF);

        // Przycisk Odrzuć
        context.fill(x + 135, btnY, x + 135 + btnW, btnY + btnH, 0xCC381A1A);
        drawBorder(context, x + 135, btnY, btnW, btnH, 0xFFFF5555);
        context.drawCenteredTextWithShadow(client.textRenderer, "§c[" + getDeclineKeyName() + "] ODRZUĆ", x + 135 + btnW / 2, btnY + 6, 0xFFFFFFFF);
    }

    private void renderMiniPrompt(DrawContext context, Interaction inv, int sw, int sh) {
        MinecraftClient client = MinecraftClient.getInstance();
        String text = "§b" + inv.getInitiatorName() + " §f» §a[" + getAcceptKeyName() + "] Akceptuj  §c[" + getDeclineKeyName() + "] Odrzuć";
    ...
    public void renderInWorldBillboard(PlayerEntity entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        String text2 = "§a[" + getAcceptKeyName() + "] §f/ §c[" + getDeclineKeyName() + "]";
