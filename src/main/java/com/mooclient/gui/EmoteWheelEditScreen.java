...
/**
 * Interactive Visual 12-Slot Drag & Drop Emote Wheel Editor.
 * Allows placing emotes directly into any of the 12 clock positions on the wheel,
 * with real-time 3D player model animation previews and instant swapping/deletion.
 */
public class EmoteWheelEditScreen extends Screen {
    ...
    private int dragSourceSlotIndex = -1; // -1 if from dock, >=0 if from wheel
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, title, cx - (titleW / 2), 18, 0xFFFFFFFF);
        context.drawTextWithShadow(this.textRenderer, subtitle, cx - (subW / 2), 32, 0xFFA0A0B0);
        ...
        // Render each of the 12 radial sectors
        for (int i = 0; i < 12; i++) {
            ...
            // If slot has an emote: render 2D pixel-art icon + [X] delete button
            if (hasEmote && def != null) {
                ...
                // Quick 1-click Delete button [X] on hover (only when not dragging)
            ...
            // If slot is empty: render "+" and slot number
            else {
                String slotNum = "#" + (i + 1);
                int snW = this.textRenderer.getWidth(slotNum);
        ...
        // Center Info Callout for hovered wheel slot
        if (hoveredWheelSlot >= 0) {
            String calloutText;
            String emoteId = workingSlots[hoveredWheelSlot];
            if (emoteId != null) {
                EmoteWheelConfig.EmoteDefinition def = EmoteWheelConfig.getDefinition(emoteId);
                calloutText = (def != null) ? MooLanguage.get("emotes_slot_prefix") + (hoveredWheelSlot + 1) + ": " + def.getDisplayName().toUpperCase() : "";
            } else {
        ...
        // Only show emotes that are NOT currently placed on any slot!
        List<EmoteWheelConfig.EmoteDefinition> availableEmotes = new ArrayList<>();
        List<String> equippedList = Arrays.asList(workingSlots);
        for (EmoteWheelConfig.EmoteDefinition def : EmoteWheelConfig.getAllEmotes()) {
            if (!equippedList.contains(def.id())) {
                availableEmotes.add(def);
            }
        }
        ...
        if (availableEmotes.isEmpty()) {
            ...
            if (targetDockScrollOffset > maxScroll) targetDockScrollOffset = maxScroll;
            if (targetDockScrollOffset < 0) targetDockScrollOffset = 0;
            dockScrollOffset = MathHelper.lerp(0.35f, dockScrollOffset, targetDockScrollOffset);

            int startX = dockX + 16;
            int cardY = dockY + 24;

            context.enableScissor(dockX + 6, dockY + 18, dockX + dockW - 6, dockY + dockH - 6);

            for (int i = 0; i < availableEmotes.size(); i++) {
                EmoteWheelConfig.EmoteDefinition def = availableEmotes.get(i);
                int cardX = (int) (startX + i * (cardW + 12) - dockScrollOffset);

                boolean isHover = mouseX >= Math.max(cardX, dockX + 6) && mouseX <= Math.min(cardX + cardW, dockX + dockW - 6) && mouseY >= cardY && mouseY <= cardY + cardH;
                if (isHover) {
                    hoveredDockEmoteId = def.id();
                }

                int cardBg = isHover ? 0xDD252538 : 0xAA181824;
                int cardBorder = isHover ? MooClientSettings.getAccentColor() : 0x44555577;

                context.fill(cardX, cardY, cardX + cardW, cardY + cardH, cardBg);
                drawRectOutline(context, cardX, cardY, cardW, cardH, cardBorder);

                // Pixel-Art Icon inside dock card
                if (def.icon() != null) {
                    context.drawTexture(RenderLayer::getGuiTextured, def.icon(), cardX + 8, cardY + 9, 0.0f, 0.0f, 32, 32, 32, 32);
                }

                // Name
                String name = def.getDisplayName();
                if (this.textRenderer.getWidth(name) > 75) {
                    name = name.substring(0, Math.min(name.length(), 10)) + "..";
                }
                context.drawTextWithShadow(this.textRenderer, name, cardX + 46, cardY + 14, 0xFFFFFFFF);

                // Access status
                String status = def.isUnlocked() ? MooLanguage.get("emotes_unlocked") : MooLanguage.get("emotes_rank_required");
                context.drawTextWithShadow(this.textRenderer, status, cardX + 46, cardY + 28, 0xFFAAAAAA);
            }

            context.disableScissor();

            // Scrollbar track & thumb when scrollable
            if (maxScroll > 0) {
                int trackX = dockX + 16;
                int trackW = dockW - 32;
        ...
        if (draggingEmoteId != null) {
            ...
            EmoteWheelConfig.EmoteDefinition dragDef = EmoteWheelConfig.getDefinition(draggingEmoteId);
            if (dragDef != null && dragDef.icon() != null) {
                context.drawTexture(RenderLayer::getGuiTextured, dragDef.icon(), dragX + 9, dragY + 9, 0.0f, 0.0f, 32, 32, 32, 32);
            }
        }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (button == 0 && draggingEmoteId != null) {
            ...
            // 2. Drop on one of the 12 exact radial slots
            else if (hoveredWheelSlot >= 0 && hoveredWheelSlot < 12) {
                if (dragSourceSlotIndex >= 0) {
                    // Swapping between two wheel positions
                    ...
                    // Dropping from dock into exact slot
    ...
    private void drawLine(DrawContext context, int x1, int y1, int x2, int y2, int color) {
        ...
        while (true) {
            context.fill(curX, curY, curX + 1, curY + 1, color);
